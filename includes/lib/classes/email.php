...
//$output['headers']['Content-Type'] = "Content-type: text/html; charset=iso-8859-1\r\n";
 ...
 */
    ...
    function send($to_name, $to_addr, $from_name, $from_addr, $subject = '', $headers = '') {
	 ...
	 // ==============================================================
		// Get TimeLimit Value From Database
		// ==============================================================
		   // $wait_query=tep_db_query("select configuration_value from configuration where configuration_title='Email Send TimeLimit'");
			//$wait_value=tep_db_fetch_array($wait_query);
			$wait_time=EMAIL_TIME_LIMIT;
			
 	// ============================================================
	// Sends E-Mails Check if contents are Valid
	// ============================================================
	   ...
	   // ============================================================
	  // Looping For Extra Headers After Check it is Exists
	  // ============================================================
     ...
     // ============================================================
	 // Check if Extra-Headers Not Empty
	 // ============================================================
	 if ($headers[$i] != '') {
    ...
    // ====================================================================
    // Mail is Send To The Recipient And Check Server is SMTP or Not
    // ====================================================================
	  //if (EMAIL_TRANSPORT == 'smtp') {
	  if ($this->smtp_array['EMAIL_TRANSPORT'] == 'smtp') {
		if(!$this->SmtpSend($this->headers,$this->html)){
			$eresult=@mail($to, $subject, $this->output, 'From: '.$from.$this->lf.implode($this->lf, $this->headers).$this->lf.implode($this->lf, $xtra_headers)); 
		}
      	 //$eresult=@mail($to_addr, $subject, $this->output, 'From: ' . $from . $this->lf . 'To: ' . $to . $this->lf . implode($this->lf, $this->headers) . $this->lf . implode($this->lf, $xtra_headers));
	  } else {
		//print_r($this);
		//exit;
		$eresult=@mail($to, $subject, $this->output, 'From: '.$from.$this->lf.implode($this->lf, $this->headers).$this->lf.implode($this->lf, $xtra_headers));
		//$eresult=@mail($to, $subject,'From: '.$from.$this->lf.implode($this->lf, $this->headers).$this->lf.implode($this->lf, $xtra_headers) . "\n\n" . $this->html);
      }

	if (!$eresult && class_exists("PHPGMailer")){ 
		$mail = new PHPGMailer();
		/*$mail->Username = base64_decode(SMTP_USER); 
		$mail->Password = base64_decode(SMTP_PASS);
		$mail->Host=SMTP_HOST;
		$mail->Port=SMTP_PORT;*/
		$mail->Username = base64_decode($this->smtp_array['user']);
		$mail->Password = base64_decode($this->smtp_array['pass']);
		$mail->Host=$this->smtp_array['server_information'];
		$mail->Port=$this->smtp_array['server_port_number'];
		$mail->From = $from_addr; 
		$mail->Subject=$subject;
		if ($this->html && $this->html!=''){
			$mail->Body=$this->html;
			if (!$this->text && $this->text=='') $this->text=strip_tags($this->html);
			$mail->AltBody=$this->text;
		} else if ($this->text && $this->text!=''){
			$mail->Body=$this->text;
		} else if ($this->html_text && $this->html_text!=''){
			$mail->Body=$this->html_text;
		}
...
function SmtpSend($header, $body) {
	...
	$smtp_from = ($this->from == "") ? $this->from : $this->from;
...
function SmtpConnect() {
	...
	$hosts = explode(";", $this->smtp_array['SMTP_HOST']);
	$index = 0;
	...
	while($index < count($hosts) && $connection == false)
	{
		...
		{
			$host = $hosts[$index];
			$port = $this->smtp_array['SMTP_PORT'];
		}
		if($this->smtp->Connect($host, $port, "30")) // 30 Timeout
		{
			if ($this->Helo != '')
			...
			if($this->SMTPAuth)
			{
				if(!$this->smtp->Authenticate($smtp_array['SMTP_USER'],
											  $smtp_array['SMTP_PASS']))
				{
					$this->SetError("en");
					$this->smtp->Reset();
					$connection = false;
...
function set_smtp($smtparray){
	$this->smtp_array['SMTP_USER'] = $smtparray['user'];
	$this->smtp_array['SMTP_PASS'] = $smtparray['pass'];
	$this->smtp_array['SMTP_HOST'] = $smtparray['server_information'];
	$this->smtp_array['SMTP_PORT'] = $smtparray['server_port_number'];
	$this->smtp_array['EMAIL_TRANSPORT'] = "smtp";
	/*if(!defined("SMTP_USER"))define('SMTP_USER',$user);
	if(!defined("SMTP_PASS"))define('SMTP_PASS',$pass);
	if(!defined("SMTP_HOST"))define('SMTP_HOST',$server_information);
	if(!defined("SMTP_PORT"))define('SMTP_PORT',$server_port_number);
	if(!defined("EMAIL_TRANSPORT"))define('EMAIL_TRANSPORT',"smtp");*/
}
