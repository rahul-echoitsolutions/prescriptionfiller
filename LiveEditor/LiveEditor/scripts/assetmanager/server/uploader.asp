...
Class Uploader
    ...
    Public Property Get allowedTypes()
		allowedTypes = privAllowedTypes
	End Property

	Public Property Let allowedTypes(sz)
		privAllowedTypes = sz
	End Property
	...
	Public Function IsAllowed(ext)
		...
		If Me.allowedTypes="" Then
			IsAllowed = True
		End If

		If(InStr(1, "|" & Me.allowedTypes & "|", "|" & ext & "|", 1)<>0) then
			IsAllowed = True
		End If
	...
	Public Sub Save(path)
		...
		For Each fileItem In UploadedFiles.Items
			...
			'added by jack
			'---------------------------------------------------
			'check if the files is allowed.
			If IsAllowed(GetExtension(fileItem.FileName)) = true Then
			'---------------------------------------------------
